...
test("builds teetime tables for weeknight event using default splits", () => {
	...
	const eastSlots = slots1.map((s) => RegistrationSlotApiSchema.parse(s)).map((s1) => new RegistrationSlot(s1))
	const northSlots = slots2.map((s) => RegistrationSlotApiSchema.parse(s)).map((s2) => new RegistrationSlot(s2))
	const westSlots = slots3.map((s) => RegistrationSlotApiSchema.parse(s)).map((s3) => new RegistrationSlot(s3))
...
test("builds teetime tables based on splits in the event settings", () => {
	const eventData = getTestEvent(TestEventType.weeknight)
	const clubEvent = new ClubEvent(ClubEventApiSchema.parse({ ...eventData, ...{ tee_time_splits: "10" } }))
	const slots1 = buildTeetimeSlots(clubEvent.id, 1, 5, 5)
	const slots2 = buildTeetimeSlots(clubEvent.id, 10, 5, 5)
	const slots3 = buildTeetimeSlots(clubEvent.id, 19, 5, 5)
	const eastSlots = slots1.map((s) => RegistrationSlotApiSchema.parse(s)).map((s1) => new RegistrationSlot(s1))
...
test("builds teetime tables for alternating splits", () => {
	const eventData = getTestEvent(TestEventType.weeknight)
	const clubEvent = new ClubEvent(ClubEventApiSchema.parse({ ...eventData, ...{ tee_time_splits: "8,9" } }))
	const slots1 = buildTeetimeSlots(clubEvent.id, 1, 5, 5)
	const slots2 = buildTeetimeSlots(clubEvent.id, 10, 5, 5)
	const slots3 = buildTeetimeSlots(clubEvent.id, 19, 5, 5)
	const eastSlots = slots1.map((s) => RegistrationSlotApiSchema.parse(s)).map((s1) => new RegistrationSlot(s1))
