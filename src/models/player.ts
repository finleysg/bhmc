...
export const GuestPlayerApiSchema = z.object({
	email: z.string().trim().min(1, "An email is required.").email("Please provide a valid email address."),
...
export class Player {
	...
	constructor(json: PlayerApiData | undefined | null, isFriend?: boolean) {
		...
		this.ageAtYearEnd = this.birthDate ? differenceInYears(lastDayOfYear(new Date()), this.birthDate) : undefined
