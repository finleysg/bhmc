...
export class ClubEvent {
	...
	constructor(json: ClubEventData) {
		...
		this.registrationWindow = json.registration_window ? (json.registration_window as RegistrationWindow) : "n/a"
	 ...
	 */
	canEditRegistration() {
		return (this.eventType === EventType.Major || this.eventType === EventType.Weeknight) && Boolean(this.paymentsEnd)
	}
	...
	playerCanRegister(now: Date, player?: Player) {
		if (!player) {
			...
			return false
		} else if (this.registrationType === RegistrationType.ReturningMembersOnly && !player.isReturningMember) {
			return false
