	...
	return (
		<aside className={sidebarOpen ? "sidebar sidebar--bg toggled" : "sidebar sidebar--bg"}>
			<ul className="navigation">
				...
				{(groups.indexOf(Groups.Administrators) >= 0 || groups.indexOf(Groups.TournamentAdmins) >= 0) && (
				...
				<MenuItem path={`calendar/${year}/${monthName.toLowerCase()}`} icon={<GoCalendar />} name="Event Calendar" />
				<MenuItem path={`champions/${currentSeason}`} icon={<GiLaurelsTrophy />} name="Champions" />
				{groups.indexOf(Groups.AuthenticatedUsers) >= 0 && (
					<MenuItem path={`my-scores/gross/${currentSeason}`} icon={<MdGridOn />} name={`My Scores`} />
				)}
