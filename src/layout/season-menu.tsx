			...
			<div className="dropdown">
				...
				<div className={`dropdown-menu dropdown-menu-right ${showMenu ? "show" : ""}`}>
					{seasons()
						...
						.map((year) => {
							return (
								<Link key={year} onClick={() => setShowMenu(false)} to={`${baseUrl}/${year}`} className="dropdown-item">
									{year}
								</Link>
							)
						})}
					{includeAll && (
						<Link key={"All"} onClick={() => setShowMenu(false)} to={`${baseUrl}/all`} className="dropdown-item">
							All
						</Link>
					)}
				</div>
