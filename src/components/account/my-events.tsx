	...
	return (
		<Link to={eventUrl()} className="d-block mb-2">
			<p className="fw-bold mb-0">
				<span>{format(registration.startDate, "MMM d, yyyy")}:</span> <span>{registration.name}</span>
			</p>
			<small className="text-secondary">
				Signed up by {registration.signedUpBy} on {format(registration.signupDate, "MM/dd/yyyy h:mm aaaa")}
			</small>
		</Link>
	)
}
