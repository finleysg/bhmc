...
export function RegistrationSlotPlayer({ slot, mode, team, onRemovePlayer }: RegistrationSlotPlayerProps) {
	return (
		<div className="player">
			{mode === "new" && slot.playerId === 0 && <span className="text-secondary">Add a player</span>}
			{slot.playerId !== 0 && (
				<React.Fragment>
					...
					{mode === "new" && slot.slot > 0 && (
						<span className="ms-2">
							<button className="btn btn-link text-success-emphasis p-0 align-top" onClick={() => onRemovePlayer(slot)}>
								(remove)
							</button>
						</span>
					)}
