...
export function DamCupResults() {
	...
	return (
		<div>
							...
							return (
								<tr key={r.season}>
									...
									<td className="text-center">
										{r.ourScore > r.theirScore && <span style={{ fontWeight: "bold" }}>{r.ourScore}</span>}
										{r.ourScore > r.theirScore || <span>{r.ourScore}</span>}
									</td>
									<td className="text-center">
										{r.ourScore < r.theirScore && <span style={{ fontWeight: "bold" }}>{r.theirScore}</span>}
			...
			<LoadingSpinner loading={status === "pending" || fetchStatus === "fetching"} paddingTop="90px" />
		</div>
	)
}
