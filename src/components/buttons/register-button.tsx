...
export function RegisterButton({ clubEvent, hasSignedUp, onClick, ...rest }: RegisterButtonProps & ClubEventProps) {
	const currentTime = new Date()
	const targetDate = clubEvent.prioritySignupStart ?? clubEvent.signupStart ?? currentTime
	const startCountdown = isBefore(currentTime, targetDate) && differenceInMinutes(targetDate, currentTime) < 60
	...
	const isDisabled = !clubEvent.playerCanRegister(currentTime, player) || !clubEvent.playerIsEligible(player)
	const buttonText =
