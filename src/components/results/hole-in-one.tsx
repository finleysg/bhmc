...
export function HoleInOneCard() {
	const { data: pageContent, status: pcStatus, fetchStatus: pcFetchStatus } = usePageContent("hole-in-one")
	const { data: aces, status, fetchStatus } = useAces(currentSeason)

	const isLoading =
		pcStatus === "pending" || pcFetchStatus === "fetching" || status === "pending" || fetchStatus === "fetching"
	...
	return (
		<div className="d-flex">
			...
			<div className="flex-grow-1">
				{user.isAuthenticated ? <Link to={`/directory/${player.id}`}>{player.name}</Link> : <span>{player.name}</span>}
			</div>
			<div>{hole}</div>
		</div>
	)
