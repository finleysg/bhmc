...
const DocumentUploadSchema = z.object({
	...
	year: z.coerce.number({ required_error: "Associate the document with a season, or enter 0." }),
})
	...
	return (
		<div>
			<form onSubmit={handleSubmit(handleFileSubmit)}>
				...
				/>
				<InputControl name="year" label="Year" register={register("year")} error={formErrors.year} type="text" />
				<InputControl name="title" label="Title" register={register("title")} error={formErrors.title} type="text" />
				<FilePicker onSelected={handleFileSelected} onDrop={handleFileSelected} />
				<div className="d-flex justify-content-end">
					<button type="button" className="btn btn-light" disabled={isSubmitting} onClick={onCancel}>
						Cancel
					</button>
					<button
						type="submit"
