...
function UpcomingEvent({ clubEvent }: ClubEventProps) {
	return (
		<Link className="listview__item mb-3" to={clubEvent.eventUrl}>
			...
			<div className={`listview__content ${clubEvent.status === EventStatusType.Canceled ? "canceled" : ""}`}>
				...
				<div className="listview">
					{upcoming().map((event) => (
						<div style={{ position: "relative" }} key={event.id}>
							...
							<AdminLinkButton to={event.adminUrl} label="Event administration home" color="transparent" />
						</div>
					))}
				</div>
			</div>
