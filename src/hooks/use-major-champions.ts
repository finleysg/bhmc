...
export function useChampions(season: number) {
	return useQuery({
		...
		queryFn: () => getMany<MajorChampionData>(`champions/?season=${season}`, MajorChampionApiSchema),
...
export function useEventChampions(clubEvent: ClubEvent) {
	return useQuery({
		...
		queryFn: () => getMany<MajorChampionData>(`champions/?event=${clubEvent.id}`, MajorChampionApiSchema),
		select: (data) => data.map((champ) => new MajorChampion(champ)),
	})
}
