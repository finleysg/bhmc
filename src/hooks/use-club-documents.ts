...
export function useClubDocumentEdit(code: string) {
	...
	const { mutateAsync: createDocument, status: status1 } = useMutation({
		mutationFn: (args: ClubDocumentArgs) => httpClient(apiUrl("documents"), { body: args.formData }),
	})
	...
	const { mutateAsync: createClubDocument, status: status3 } = useMutation({
		mutationFn: (args: ClubDocumentArgs) => httpClient(apiUrl("static-documents"), { body: args.formData }),
	})
	...
	const handleUpload = async (existingDocument: ClubDocument | null, documentData: DocumentUploadData, file: File) => {
		setError(null)
	...
	return {
		handleUpload,
		error,
		isBusy: status1 === "pending" || status2 === "pending" || status3 === "pending" || status4 === "pending",
	}
}
