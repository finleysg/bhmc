...
export function useAdminPaymentData(registrationId: number, paymentId: number): AdminPaymentValidation {
	const { user } = useAuth()
	...
	const {
		...
		queryFn: () => getOne<RegistrationData>(`registration/${registrationId}/`, RegistrationApiSchema),
