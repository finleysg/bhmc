...
export function ReserveManager1({ clubEvent }: ClubEventProps) {
	...
	const { mutateAsync: registerPlayer, status: registerStatus, error: registerError } = useRegisterPlayer(clubEvent.id)
	...
	const handleMove = async (registrationId: number, sourceSlots: ReserveSlot[], destinationSlots: ReserveSlot[]) => {
		await movePlayers({
			registrationId,
	...
	const handleDrop = async (registrationId: number, slotIds: number[], refunds: Map<number, RefundData>) => {
		try {
			setBusy(true)
