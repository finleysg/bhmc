 ...
 * @returns
 */
const createRefundDetails = (slots: ReserveSlot[], payments: PaymentData[], eventFeeMap: Map<number, EventFee>) => {
	...
	const filteredPayments = payments.filter(
		(p) =>
			p.confirmed &&
			p.payment_details &&
			p.payment_details.some((d) => d.registration_slot && slotIds.indexOf(d.registration_slot) >= 0),
	)
	slots.forEach((slot) => {
	...
	return (
		<div className="card border border-danger">
			<div className="card-body">
				...
				<div className="fw-bold text-danger-emphasis text-end">Refund amount: ${refundAmount()}</div>
