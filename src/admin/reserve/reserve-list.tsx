...
export function ReserveListAdmin({
	...
	const handleAddConfirm = (player: Player, feeIds: number[], isMoneyOwed: boolean, notes: string) => {
		try {
	...
	const handleDropConfirm = (dropSlots: ReserveSlot[], dropNotes: string) => {
		try {
			const registrationId = selectedRegistrationId > 0 ? selectedRegistrationId : dropSlots[0].registrationId
			if (!registrationId) {
					...
					<div className="card-body">
						...
						{reservations.map((res) => (
							<ReservationAdmin
								key={res.slotId}
								reservation={res}
								selected={selectedSlotId === res.slotId || selectedRegistrationId === res.registrationId}
						...
						<Modal show={showNotes}>
							{selectedRegistrationId && (
								<EditNotes
									signedUpBy={registrations.find((r) => r.id === selectedRegistrationId)?.signedUpBy ?? ""}
						...
						<Modal show={showChange}>
							{selectedRegistrationId && (
								<ChangeEvent onChange={handleChangeEventConfirm} onCancel={handleChangeEventCancel} />
							)}
						</Modal>
					</div>
				</div>
