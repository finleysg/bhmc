...
export function ReserveCardAdmin({
	...
	if (!moveStarted) {
		if (reserveSlot.status === RegistrationStatus.Reserved) {
			...
			return (
				<div className={reserveSlot.deriveClass()} style={{ cursor: "pointer" }} onClick={onSlotSelect} {...rest}>
		...
		if (reserveSlot.status === RegistrationStatus.Reserved) {
			...
			return (
				<div className={reserveSlot.deriveClass()} style={{ cursor: "move" }} onClick={onMoveSelect} {...rest}>
					open
				</div>
			)
		}
