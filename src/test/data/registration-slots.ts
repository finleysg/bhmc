...
export const buildTeetimeSlots = (eventId: number, holeId: number, groups: number, groupSize: number) => {
	const slots = []
...
export const buildShotgunSlots = (eventId: number, holeId: number, groups: number, groupSize: number) => {
	const slots = []
	for (let i = 0; i < groups * groupSize; i++) {
		const slot = buildSlot({
			overrides: {
